...
**포함된 내용**:
- AWS: VPC, RDS, ECS, ALB(HTTPS), **Route 53(도메인)**
...
4. [Step 4: 백엔드 서버(ECS) 배포](#step-4-백엔드-서버ecs-배포)
5. [Step 5: 로드밸런서(ALB) 및 도메인 연결](#step-5-로드밸런서alb-및-도메인-연결)
6. [Step 6: 프론트엔드 배포(S3+CloudFront)](#step-6-프론트엔드-배포s3cloudfront)
7. [Step 7: GCP 의사(Cloud Run) 배포](#step-7-gcp-의사cloud-run-배포)
...
## Step 4: 백엔드 서버(ECS) 배포

### 4-1. ECR 생성 및 이미지 푸시 (로컬 PC에서 수행)
