...
/**
 * GET /api/auth/test-bcrypt
 * bcrypt 테스트 엔드포인트
 */
router.get('/test-bcrypt', async (req, res) => {
  console.log('[AUTH] Testing bcrypt...');
  try {
    const bcrypt = require('bcryptjs');
    const testPassword = 'test123';

    console.log('[AUTH] Generating salt...');
    const salt = await bcrypt.genSalt(10);
    console.log('[AUTH] Salt generated:', salt);

    console.log('[AUTH] Hashing password...');
    const hash = await bcrypt.hash(testPassword, salt);
    console.log('[AUTH] Hash generated:', hash);

    console.log('[AUTH] Comparing password...');
    const isMatch = await bcrypt.compare(testPassword, hash);
    console.log('[AUTH] Compare result:', isMatch);
    ...
    console.error('[AUTH] bcrypt test failed:', error);
    res.status(500).json({
      success: false,
      error: error.message,
      stack: error.stack
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    console.log('[AUTH] Parsed: email=%s, password=%s, name=%s', email, password ? '***' : 'empty', name);
