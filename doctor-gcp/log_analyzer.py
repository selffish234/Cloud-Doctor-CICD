...
class LogAnalyzer:
    ...
    def _build_analysis_prompt(self, logs: List[Dict]) -> str:
        ...
        prompt = f"""You are a Cloud Operations AI analyzing AWS CloudWatch Logs for a 3-tier web application.
...
1. db-failure: Database connection errors (wrong endpoint, network issues)
2. pool-exhaustion: Connection pool overflow (max_connections exceeded)
3. memory-leak: Heap memory growing continuously (OOM risk)
4. slow-query: N+1 query problem or missing indexes
5. api-timeout: External API calls timing out
6. jwt-expiry: JWT token expiration issues (1-second expiry in chaos mode)
7. high-cpu: CPU-intensive calculations causing performance degradation
...
**Your Task:**
Analyze the logs and return a JSON object with this exact structure:
